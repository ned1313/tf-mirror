  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <header class="bg-white shadow">
      <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-600">Browse Cached Modules</p>
        </div>
    ...
    <main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          ...
          <div class="flex flex-wrap items-center gap-4">
            <div class="flex-1 min-w-64">
              <input
                v-model="searchQuery"
                type="text"
                placeholder="Search modules (e.g., hashicorp/consul/aws)..."
      ...
      <div class="px-4 sm:px-0">
        ...
        <div v-else-if="filteredModules.length === 0" class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
          ...
          <p class="mt-1 text-sm text-gray-500">
            {{ searchQuery || namespaceFilter || systemFilter ? 'Try adjusting your search filters' : 'No modules have been cached yet' }}
          </p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
            ...
            :key="`${module.namespace}-${module.name}-${module.system}`"
            ...
            <div class="flex items-start justify-between">
              <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                  {{ module.system }} • {{ module.versions.length }} version(s)
                </p>
              </div>
        ...
        <div v-if="totalPages > 1" class="mt-6 flex items-center justify-between">
          <div class="text-sm text-gray-700">
            Showing {{ (currentPage - 1) * pageSize + 1 }} to {{ Math.min(currentPage * pageSize, filteredModules.length) }} of {{ filteredModules.length }}
          </div>
          ...
          <div class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform sm:my-8 sm:max-w-2xl sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
              <div class="flex items-start justify-between mb-4">
                <div>
                  <h3 class="text-lg font-medium text-gray-900">
                    {{ selectedModule.namespace }}/{{ selectedModule.name }}/{{ selectedModule.system }}
                  </h3>
                  <p class="text-sm text-gray-500">Terraform Module</p>
                </div>
              ...
              <div class="space-y-4">
                ...
                <div>
                  <h4 class="text-sm font-medium text-gray-500">Terraform Usage</h4>
                  <pre class="mt-2 p-3 bg-gray-100 rounded-md text-xs overflow-x-auto">module "{{ selectedModule.name }}" {
  source  = "{{ mirrorHost }}/{{ selectedModule.namespace }}/{{ selectedModule.name }}/{{ selectedModule.system }}"
  version = "{{ selectedModule.versions[0] }}"
                ...
                <div>
                  ...
                  <code class="mt-1 block p-2 bg-gray-100 rounded text-xs break-all">
                    {{ mirrorHost }}/v1/modules/{{ selectedModule.namespace }}/{{ selectedModule.name }}/{{ selectedModule.system }}/
                  </code>
                </div>
              </div>
            </div>
...
const filteredModules = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(m =>
      ...
      m.system.toLowerCase().includes(query)
    )
  }
...
const namespaces = computed(() => {
  const ns = new Set(modules.value.map(m => m.namespace))
  return Array.from(ns).sort()
})

const systems = computed(() => {
  const sys = new Set(modules.value.map(m => m.system))
  return Array.from(sys).sort()
})

const totalPages = computed(() => Math.ceil(filteredModules.value.length / pageSize))
...
watch([searchQuery, namespaceFilter, systemFilter], () => {
  currentPage.value = 1
})
...
async function fetchModules() {
  loading.value = true
  try {
    const response = await fetch('/api/public/modules?page_size=1000')
    if (response.ok) {
